...
namespace Busidex.Professional.Views
{
	...
	public partial class ShareView
	{
        ...
        private readonly bool _shareOwner = false;

        public ShareView(ref UserCard uc, bool shareOwner = false)
	    {
	        InitializeComponent();
        ...
        private async void BtnShare_OnClicked(object sender, EventArgs e)
        {
            ...
            {
                _viewModel.MessageSent = await sendSMSAsync();
            }
            else
            {
                _viewModel.MessageSent = await sendEmailAsync();
            }
        }
