...
namespace Busidex.Professional.ViewModels
{
    ...
    public class CardVM : BaseViewModel
    {
        ...
        public ObservableRangeCollection<PhoneNumberVM> PhoneNumbers {
            get => _phoneNumbers;
            set {
                _phoneNumbers = value;
        ...
        public ImageSource AddPhoneImage {
            get => _addPhoneImage;
            set {
        ...
        public string SelectedCardFrontImage {
            get => _selectedCardFrontImage;
            set {
        ...
        public string SelectedCardBackImage {
            get => _selectedCardBackImage;
            set {
        ...
        public void SaveThumbnail(string imageFile, string fileId, string orientation)
        {
            if (!File.Exists(imageFile)) throw new FileNotFoundException(imageFile);
            var bytes = File.ReadAllBytes(imageFile);
            
            var height = orientation == "H" ? 152 : 250;
            var width = orientation == "H" ? 250 : 152;

            var smallerBytes = Plugin.ImageResizer.CrossFeature1.Current.Resize(height, width, bytes);
            var scaledImageFile = $"{Serialization.LocalStorageFolder}/{StringResources.THUMBNAIL_FILE_NAME_PREFIX}{fileId}.jpg";

            File.WriteAllBytes(scaledImageFile, smallerBytes);
        ...
        public async Task<bool> SaveCardImage()
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(EncodedFrontCardImage))
                {
                    var filePath = Path.Combine(storagePath, SelectedCard.Card.FrontFileName);
                    var bytes = File.ReadAllBytes(filePath);
                    EncodedFrontCardImage = bytes.Length > 0 ? Convert.ToBase64String(bytes) : string.Empty;
                }
                ...
                var card = new MobileCardImage
                {
                    ...
                    EncodedCardBackImage = EncodedBackCardImage,
                    FrontOrientation = FrontOrientation,
                    BackOrientation = BackOrientation
                };

                var frontResult = await _cardHttpService.UpdateCardImage(card);
        ...
        public async Task<bool> SaveAddress()
        {
            try
            {
                ...
                var result = await _cardHttpService.UpdateCardAddress(json);
                if (result)
                {
                    var resp = await _cardHttpService.GetCardById(SelectedCard.Card.CardId);
                    if(resp.Model.Addresses.Count > 0)
                    {
        ...
        public async Task<bool> SaveExternalLinks()
        {
            try
            {
                ...
                var json = JsonConvert.SerializeObject(linksModel);
                var result = await _cardHttpService.UpdateCardLinks(json);
        ...
        public async Task<bool> SaveTags()
        {
            try
            {
                ...
                var result = await _cardHttpService.UpdateCardTags(json);
                if (result)
                {
                    var resp = await _cardHttpService.GetCardById(SelectedCard.Card.CardId);
        ...
        public async Task<bool> SaveSearchInfo()
        {
            try
            {
                ...
                var searchInfo = new CardSearchInfo()
                {
                    CardId=SelectedCard.CardId,
                ...
                var result = await _cardHttpService.UpdateCardSearchInfo(json);
                if (result)
                {
                    var resp = await _cardHttpService.GetCardById(SelectedCard.CardId);
        ...
        public async Task<bool> SaveContactInfo()
        {
            ...
            {
                AllowSave = false;
                PhoneNumbers.RemoveAll(p => string.IsNullOrEmpty(p.Number));

                var pnList = new List<PhoneNumber>();
                pnList.AddRange(PhoneNumbers.Select(p => new PhoneNumber
                {
                    CardId = SelectedCard.Card.CardId,
                ...
                pnList.AddRange(deleted);

                var contactInfo = new CardContactInfo()
                {
                    CardId = SelectedCard.Card.CardId,
                    PhoneNumbers = new List<PhoneNumber>(),
                    Email = SelectedCard.Card.Email,
                    Url = SelectedCard.Card.Url
                };
                contactInfo.PhoneNumbers.AddRange(pnList);
                var json = JsonConvert.SerializeObject(contactInfo);
                var result = await _cardHttpService.UpdateCardContactInfo(json);
                if (result)
                {
                    var resp = await _cardHttpService.GetCardById(SelectedCard.Card.CardId);
